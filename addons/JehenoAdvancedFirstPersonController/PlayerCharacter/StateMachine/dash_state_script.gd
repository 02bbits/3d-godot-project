...
func verifications():
	...
	play_char.dash_direction = Vector3(play_char.move_direction.x, 0.0, play_char.move_direction.z).normalized() #get move direction before actually start dashing, and stick to that direction (horizontal only)
