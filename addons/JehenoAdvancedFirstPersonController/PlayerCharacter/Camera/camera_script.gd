...
@export_range(0.0, 0.5, 0.001) var x_axis_sensibility : float = 0.025
@export_range(0.0, 0.5, 0.001) var y_axis_sensibility : float = 0.025
...
@export_range(-180.0, 180.0, 1.0) var zoom_val : float = 50.0
...
@export_range(0.0, 6.0, 0.1) var side_move_tilt_divider : float = 0.8
@export_range(0.0, 24.0, 0.01) var side_move_tilt_speed : float = 5.0
@export_range(0.0, 12.0, 0.001) var side_move_max_tilt_val : float = 9.0
...
@export var enable_headbob : bool = false
...
func tilt(delta : float) -> void:
	if state != "Slide" and state != "Wallrun":
...
func bob(delta : float) -> void:
	...
	if enable_headbob and state != "Idle" and state != "Jump" and state != "Slide" and state != "Dash" and state != "Wallrun" and !play_char.ceiling_check.is_colliding():
	...
	elif enable_headbob and (state == "Idle" or state == "Jump" or state == "Slide" or state == "Dash" or state == "Wallrun" or play_char.ceiling_check.is_colliding()):
